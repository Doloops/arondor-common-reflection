...
public class JavaAccessibleClassParser implements AccessibleClassParser
{
    ...
    private void setAccessibleClassAnnotations(Class<?> clazz, AccessibleClassBean accessClass)
    {
        for (Annotation annotation : clazz.getAnnotations())
        {
            ...
            {
                LOG.debug("Dubious and deprecated annotation " + annotation.annotationType() + " set on class "
                        + clazz.getName());
                continue;
            }
            ...
            try
            {
                ...
                for (Method m : annotationMethods)
                {
                    if (m.getName().equals("value"))
                    {
                        ...
                        LOG.debug("Class " + clazz.getName() + " : Annotation " + annotation.annotationType().getName()
                                + "=" + value);
